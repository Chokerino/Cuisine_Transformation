...
for i in ing_list_test_me:
  if(len(i)>=7):
    rand=random.sample(range(0, len(i)-1),1)
    temp2=""
    temp3=[]
...
with open('ing_cat/ing_list_italian.txt', 'w') as fp:
    for i in ing_list_train_me:
        fp.write(i+"\n")

with open('ing_cat/vocab.txt', 'w') as fp:
    for i in vocab:
        fp.write(i+"\n")


paths = [str(x) for x in Path(".").glob("ing_cat/vocab.*")]
tokenizer = CharBPETokenizer()
tokenizer.train("ing_cat/vocab.txt",vocab_size=20395,special_tokens=[
    "[CLS]",
    "[SEP]",
    "[UNK]",
    "[MASK]",
...
tokenizer.save_model("EsperBERTo_italian")
...
#tokenizer = CharBPETokenizer(
#    "./EsperBERTo_italian/vocab.txt",)

tokenizer._tokenizer.post_processor = BertProcessing(("[SEP]", tokenizer.token_to_id("[SEP]")), ("[CLS]", tokenizer.token_to_id("[CLS]")),)

print(tokenizer.encode("water butter sugar cinnamon nutmeg salt couscous almond goldenraisin date butter sugar [MASK]"))
print(tokenizer.encode("water butter sugar cinnamon nutmeg salt couscous almond goldenraisin date butter sugar [MASK]").tokens)
...
tokenizer = RobertaTokenizerFast.from_pretrained("./EsperBERTo_italian", max_len=512)
...
dataset = LineByLineTextDataset(
    tokenizer=tokenizer,
    file_path="ing_cat/ing_list_italian.txt",
    block_size=128,
)
...
trainer.save_model("./EsperBERTo_italian")

from transformers import pipeline

fill_mask = pipeline(
    "fill-mask",
    model="./EsperBERTo_italian",
    tokenizer="./EsperBERTo_italian"
)
#print(fill_mask(0)[0]["token_str"])

predictions=[]
for i in ing_list_test_me: 
  temp=[]  
  temp.append(fill_mask(i)[0]["token_str"])
...
for n,i in enumerate(y_test):
    if predictions[n]==y_test[n]:
        score+=1
print("perfect match",score,score/len(y_test))
checking={}
